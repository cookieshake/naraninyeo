...
class MessageUseCase:
    ...
    async def get_recent_messages(self, message: Message, limit: int) -> list[Message]:
        return await self.message_repository.get_surrounding_messages(
            message=message, before=limit, after=0
        )
    ...
    async def get_similar_messages(self, message: Message, limit: int = 3) -> list[list[Message]]:
        ...
        blocks = [
            asyncio.create_task(self.message_repository.get_surrounding_messages(
                message=msg, before=3, after=3
