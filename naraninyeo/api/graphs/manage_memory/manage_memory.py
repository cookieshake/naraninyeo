...
async def prune_memories(
    ...
    pruned_memories = []
    prune_actions = await memory_pruner.run_with_generator(
        MemoryPrunerDeps(memories=memories)
    )
        ...
        if action.method == "delete":
            ...
            merged_memory = MemoryItem(
                ...
                created_at=max([
                    ...
                    if memory.memory_id in action.ids
                ]),
                updated_at=now,
    ...
    memories[:] = [
        ...
        if memory.memory_id not in memory_ids_to_delete
    ]
...
async def prune_short_term_memories(
    ...
    pruned_memories = await prune_memories(
        ...
        runtime,
        short_memories,
        new_memory_retention_days=7,
        new_memory_kind="short_term"
    )
...
async def prune_long_term_memories(
    ...
    pruned_memories = await prune_memories(
        ...
        runtime,
        long_memories,
        new_memory_retention_days=60,
        new_memory_kind="long_term"
    )
    return pruned_memories
...
async def manage_memory(
    ...
    memories = await repo.get_channel_memory_items(
        ...
        state.incoming_message.channel.channel_id,
        limit=500
    )
    short_term_memories = [
        memory for memory in memories if memory.kind == "short_term"
    ]
    if len(short_term_memories) > 100:
        short_term_memories = await prune_short_term_memories(
            state,
            runtime,
            short_term_memories
        )
    old_short_memories = [
        ...
        if (runtime.context.clock.now() - m.created_at).days > 4
    ]
    if len(old_short_memories) > 30:
        promoted_short_memories = await prune_short_term_memories(
            state,
            runtime,
            short_term_memories
        )
        short_term_memories = [
            m for m in short_term_memories
            if m.memory_id not in {pm.memory_id for pm in promoted_short_memories}
        ]

    long_term_memories = [
        memory for memory in memories if memory.kind == "long_term"
    ]
    if len(long_term_memories) > 100:
        long_term_memories = await prune_long_term_memories(
            state,
            runtime,
            long_term_memories
        )
    ...
    await repo.delete_many(
        ...
        [memory.memory_id for memory in memories
         if memory.memory_id not in {nm.memory_id for nm in new_memories}]
    )
