...
@tracer.start_as_current_span("search_similar_message_clusters")
async def search_similar_message_clusters(channel_id: str, embeddings: list[float]) -> list[list[Message]]:
    """
    주어진 채널에서 주어진 임베딩과 유사한 메시지를 검색하고,
    관련 대화 클러스터를 만들어 반환합니다.
    """
    similar_message_ids = await search_similar_message_ids(channel_id, embeddings)
